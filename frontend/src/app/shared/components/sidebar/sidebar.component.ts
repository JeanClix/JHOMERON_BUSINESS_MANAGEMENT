  ...
  template: `
    ...
    >
      ...
      <div>
        ...
        <div class="mb-5 px-3">
          <div
            class="rounded-xl bg-[#ef0606] px-3.5 py-2 flex items-center text-sm font-bold text-white shadow-xs"
            [class.justify-center]="isCollapsed()"
            [class.gap-2]="!isCollapsed()"
            [title]="moduleName()"
          >
            <i [class]="moduleIcon() + ' text-sm'"></i>
            @if (!isCollapsed()) {
        ...
        <!-- NAVEGACIÓN -->
        <nav class="space-y-5 px-4">
          @for (group of navGroups(); track group.groupLabel) {
            <div>
              @if (!isCollapsed()) {
                <div class="mb-2.5 flex items-center justify-between">
                  <span class="text-xs font-bold uppercase tracking-wider text-slate-200 opacity-80"> {{ group.groupLabel }} </span>
                  <span class="rounded bg-[#ef0606] px-2 py-0.5 text-[10px] font-bold text-white shadow-xs uppercase"> {{ moduleName() }} </span>
                </div>
              }
              ...
              <div class="space-y-2">
                @for (item of group.items; track item.id) {
                  <a
                    ...
                    class="group relative flex w-full items-center gap-3 rounded-xl px-3.5 py-3.5 text-left transition-all duration-200 select-none text-slate-100 hover:bg-white/10 hover:text-white font-medium"
                    [class.justify-center]="isCollapsed()"
                    [title]="isCollapsed() ? item.label : ''"
                  >
                    <!-- Icon Container -->
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-lg transition-colors duration-150 bg-white/10 text-white group-[.active-nav-item]:bg-white group-[.active-nav-item]:text-[#ef0606] group-hover:bg-[#ef0606] group-hover:text-white">
                    ...
                    @if (!isCollapsed()) {
                      <div class="flex flex-1 flex-col overflow-hidden min-w-0">
                        ...
                        <span class="truncate text-xs font-normal group-[.active-nav-item]:text-slate-100 text-slate-300">
                          Acceder a módulo
                        </span>
                      </div>
                    }
      ...
      <div class="mt-auto flex flex-col gap-3 border-t border-white/10 pt-3 pb-4 px-4">
        ...
        @if (!isCollapsed()) {
          <div class="rounded-xl border border-white/10 bg-white/10 p-3.5 backdrop-blur-xs">
            <div class="flex items-center gap-3">
              <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#ef0606] text-sm font-bold text-white shadow-xs">
                {{ authService.currentUser()?.name?.charAt(0) || 'U' }}
              </div>
              <div class="flex min-w-0 flex-1 flex-col">
                <span class="truncate text-sm font-bold text-white">{{ authService.currentUser()?.name || 'Usuario' }}</span>
                <span class="flex items-center gap-1 truncate text-xs font-semibold text-emerald-300">
            ...
            <button
              ...
              class="mt-3 w-full flex items-center justify-center gap-2 py-2 px-2.5 rounded-lg bg-white/10 hover:bg-rose-600/80 text-rose-200 hover:text-white text-xs font-semibold border border-white/10 hover:border-rose-500/50 transition-all cursor-pointer"
              title="Cerrar sesión"
            >
