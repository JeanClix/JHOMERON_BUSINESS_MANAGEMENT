  ...
  template: `
    ...
    >
      ...
      <div>
        ...
        <!-- MODULE SWITCHER (Ventas vs Gerencia Hub) -->
        <div class="mb-4 px-3">
          @if (!isCollapsed()) {
            <div class="rounded-xl bg-white/10 p-1 flex gap-1 text-xs">
              ...
              >
                <i class="fa-solid fa-chart-pie text-amber-400 text-xs"></i> Gerencia
              </a>
            </div>
          } @else {
            <a
            ...
            <div>
              @if (!isCollapsed()) {
                <div class="mb-2 flex items-center justify-between">
                  ...
                  <span class="rounded bg-[#ef0606] px-2 py-0.5 text-[9px] font-bold text-white shadow-xs"> GERENCIA </span>
                </div>
              }
                    ...
                    @if (!isCollapsed()) {
                      <div class="flex flex-1 flex-col overflow-hidden min-w-0">
                        <div class="flex items-center justify-between gap-1.5">
                          ...
                          @if (item.badge) {
                            <span class="rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider group-[.active-nav-item]:bg-white group-[.active-nav-item]:text-[#ef0606] bg-[#ef0606] text-white">
                              {{ item.badge }}
                            </span>
                          }
                        </div>
        ...
        @if (!isCollapsed()) {
          <div class="rounded-xl border border-white/10 bg-white/10 p-3 backdrop-blur-xs">
            <div class="flex items-center gap-3">
              ...
              <div class="flex min-w-0 flex-1 flex-col">
                <span class="truncate text-xs font-bold text-white">{{ authService.currentUser()?.name || 'Dirección Gerencial' }}</span>
                <span class="flex items-center gap-1 truncate text-[11px] font-semibold text-emerald-300">
                  <span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span> {{ authService.currentUser()?.area || 'CEO / Comercial' }}
                </span>
              </div>
            </div>
          ...
          <div class="flex flex-col items-center gap-2">
            <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-[#ef0606] text-xs font-bold text-white shadow-xs"
              [title]="authService.currentUser()?.name || 'Perfil Gerencial'">
              {{ authService.currentUser()?.avatarInitials || 'DG' }}
            </div>
            <button
              type="button"
